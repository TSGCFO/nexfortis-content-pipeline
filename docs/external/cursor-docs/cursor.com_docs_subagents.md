...
| `model` | string | No | `inherit` | Model to use: `inherit` or a specific model ID. See [model configuration](https://cursor.com/docs/subagents#model-configuration). |
...
### [Model configuration](https://cursor.com/docs/subagents\#model-configuration)
...
| A specific model ID | Uses the exact model you specify, such as `composer-2` or `gpt-5.5`. See the [models reference](https://cursor.com/docs/models-and-pricing) for available IDs. |
...
#### [When the configured model won't be used](https://cursor.com/docs/subagents\#when-the-configured-model-wont-be-used)
...
```
---name: reasoning-agentdescription: Handles complex architectural decisions.model: gpt-5.5---Analyze the architecture and recommend changes with detailed reasoning.
```
...
[Cursor LogoTry in Cursor](cursor://anysphere.cursor-deeplink/prompt?text=Create%20a%20subagent%20file%20at%20.cursor%2Fagents%2Fverifier.md%20with%20YAML%20frontmatter%20containing%20name%20and%20description.%20The%20description%20should%20be%20%27Validates%20completed%20work.%20Use%20after%20tasks%20are%20marked%20done%20to%20confirm%20implementations%20are%20functional.%27%20The%20prompt%20body%20should%20instruct%20it%20to%20be%20skeptical%2C%20verify%20implementations%20actually%20work%20by%20running%20tests%2C%20and%20look%20for%20edge%20cases.)
